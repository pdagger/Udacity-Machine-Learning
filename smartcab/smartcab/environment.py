...
class Environment(object):
    ...
    def create_agent(self, agent_class, *args, **kwargs):
        ...
        self.agent_states[agent] = {'location': random.choice(list(self.intersections)), 'heading': (0, 1)}
        return agent
    ...
    def reset(self, testing=False):
        ...
        for traffic_light in iter(self.intersections.values()):
            traffic_light.reset()

        # Pick a start and a destination
        start = random.choice(list(self.intersections))
        destination = random.choice(list(self.intersections))
        ...
        # Initialize agent(s)
        for agent in iter(self.agent_states):
            ...
            # intersections and headings still in 'positions'
                ...
                intersection = random.choice(list(positions))
    ...
    def step(self):
        ...
        for intersection, traffic_light in iter(self.intersections.items()):
    ...
    def sense(self, agent):
        ...
        left = None
        right = None
        for other_agent, other_state in iter(self.agent_states.items()):
